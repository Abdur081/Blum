...
class HPV_Blum:
    ...
    def Get_Info(self) -> dict:
        ...
        try:
            ...
            Balance = HPV['availableBalance'] # Current balance
    ...
    def Play(self) -> None:
        ...
        try:
            ...
            self.Logging('Success', '🟢', 'The game has started, waiting 30-35 seconds...')
            ...
            self.Logging('Error', '🔴', 'The game is not over!')
                    ...
                    if _Task['status'] == 'NOT_STARTED': # Если задание ещё не начато
                        if self.Start_Tasks(_Task['id']):
                            ...
                            if Claim_Tasks['Status']:
                                self.Logging('Success', '⚡️', f'Mission accomplished! +{Claim_Tasks["Reward"]}')
            ...
            try:
                if self.Token: # Если аутентификация успешна
                    ...
                    if Farming: # Если фарминг ещё продолжается
                        ...
                        self.Logging('Warning', '⏳', f'Сбор уже производился! Next collection: {Waiting_STR}!')
                    ...
                    self.Logging('Success', '💰', f'Current balance: {self.Get_Info()["Balance"]}')
                    self.Logging('Warning', '⏳', f'Next collection: {Waiting_STR}!')
